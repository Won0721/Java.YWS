...
public class MathEx {
	public static void main(String[] args) {
		...
		System.out.println(Math.ceil(1.29)); // 1.29 소수점 첫번째 자리에서 올림
		System.out.println(Math.floor(1.29)); // 1.29의 소수점 첫번째 자리에서 내림
		System.out.println(Math.round(1.29)); // 1.29에서 소수점 첫번째 자리에서 반올림 
		System.out.println(Math.pow(2, 3)); // 2의 3 제곱
		System.out.println(Math.random()); // 0이상 1미만의 난수
		// 1.29의 소수점 둘째자리에서 반올림
		double num = 1.29;
		int pos = 2; // 소수점 pos자리에서 반올림
		num *= Math.pow(10, pos-1); // 1.29 
